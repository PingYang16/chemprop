...
clusters = kmeans.fit_predict(principalComp)

representative_indices = []
for cluster_id in range(n_cluster):
    cluster_indices = np.where(clusters == cluster_id)[0]
    centroid = kmeans.cluster_centers_[cluster_id]
    distances_to_centroid = np.linalg.norm(principalComp[cluster_indices] - centroid, axis=1)
...
top_50_mols.to_csv('top_50_mols_PCA.csv', index=False)